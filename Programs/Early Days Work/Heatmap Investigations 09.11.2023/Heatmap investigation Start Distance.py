...
#setup
sample_length = 1.1 #in seconds/increment
start = 230 #start variable
stop =  360 #end variable
increment = 5 #difference between 1 point and the next point in variable under investigation
...
for i in range(int(start/increment),int(stop/increment)): #cannot get decimals so will build this into code/
    car1 = Car(1, 400, 250, 15, 1, 5,1000, 'blue', 30, reaction,0) #reset car each interation
    car2 = Car(1, 400, 200, 30, 1, 5, 1000, 'green', 30, reaction, 1)
    ...
    while run:
        ...
        car1.acceleration = accel(car1.speed, 0.67, 0, 100000000000000000000, car1.speed_des, decel_max) # calculates acceleration
        ...
        car2.acceleration = accel(car2.speed, car2.t_react, del_v2, del_s2, car2.speed_des, decel_max)
        car2.suvat()
        car2.pos_update()
        ...
        #car1.draw()
        #car2.draw()
        Z[int(stop/increment)-1-i,n] = car2.acceleration #f(x,y) #enters information from the bottom row of the array instead of the top row
        #pygame.display.flip()
        n += 1
        if n > frame_tot-1:
            break
    variable.append(r_variable-200)
...
#fig, ax = plt.subplots()
plt.contour(X,Y, Z, linewidths = 0.5, colors = 'k', levels = [-10,-5])
plt.imshow(Z, cmap = cm.gray, aspect = 'auto', interpolation = 'none', extent = [min(time_tick(dt,frame_tot)),max(time_tick(dt,frame_tot)), min(variable), max(variable)], vmin = -10, vmax = -2)
